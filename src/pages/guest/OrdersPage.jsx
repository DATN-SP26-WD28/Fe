...
// --- CẤU HÌNH SEPAY ---
const SEPAY_BANK_ACCOUNT = '0355375020'
...
const OrdersPage = () => {
  ...
  // Only count money for items that are served; canceled items are excluded
  const servedItems = useMemo(() => {
    return flattenedItems.filter(
      (item) => normalizeOrderStatus(item.status || item.orderStatus) === ORDER_ITEM_STATUS.served
    )
  }, [flattenedItems])

  const servedTotalItems = useMemo(
    () => servedItems.reduce((sum, item) => sum + (item.quantity || 0), 0),
    [servedItems]
  )

  const servedGrandTotal = useMemo(() => {
    return servedItems.reduce((sum, item) => {
      const itemPrice = item.price || item.dish_id?.dish_price || 0
      return sum + itemPrice * item.quantity
    }, 0)
  }, [servedItems])

  // Determine whether payment is allowed: enabled only when ALL items are served or canceled
  const canPay = useMemo(() => {
    if (!flattenedItems || flattenedItems.length === 0) return false
    return !flattenedItems.some((item) => {
      const status = normalizeOrderStatus(item.status || item.orderStatus)
      return status === ORDER_ITEM_STATUS.pending || status === ORDER_ITEM_STATUS.confirmed
    })
  }, [flattenedItems])
  ...
  const handleProcessPayment = async () => {
    try {
      ...
      if (paymentMethod === 'sepay') {
        ...
        const res = await paymentAPI.createUrl({ table_id: tableId, amount: servedGrandTotal })
  ...
  const qrCodeUrl = `https://qr.sepay.vn/img?acc=${SEPAY_BANK_ACCOUNT}&bank=${SEPAY_BANK_NAME}&amount=${servedGrandTotal}&des=${paymentContent}&template=compact`
    ...
    <>
      ...
      <div className="min-h-screen bg-white text-slate-900 pb-40">
          ...
          {flattenedItems.length === 0 ? (
            <div className="rounded-2xl bg-white p-12 shadow-sm border text-center">
              <Empty
                description={
                  <div>
                    <p className="mt-4 text-base font-semibold">Chưa có đơn hàng nào</p>
                  </div>
                }
              />
              <button
                onClick={() => navigate(`/table-order/${tableId}/menu`)}
                className="mt-8 bg-orange-500 px-6 py-3 rounded-xl text-white font-semibold shadow-md hover:bg-orange-600"
              >
                Gọi thêm món
              </button>
            </div>
            ...
            <>
              <div className="mb-6 rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                <h3 className="font-bold text-sm mb-4 uppercase tracking-widest text-slate-900">
                  Chi tiết món ăn
                </h3>
                <div className="space-y-4">
                  {groupedItems.map((item, idx) => (
                    ...
                    >
                      <img
                        src={item.dish_id?.image_url || 'https://via.placeholder.com/80'}
                        alt="Món ăn"
                        className="h-24 w-24 rounded-lg object-cover border border-slate-200"
                      />
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-900 text-sm">
                          {item.dish_id?.dish_name}
                        </p>
                        <span className="text-sm text-slate-600">Số lượng: {item.quantity}</span>
                        <div className="mt-1">
                          <Tag
                            color={ORDER_ITEM_STATUS_MAP[normalizeOrderStatus(item.status)]?.color}
                          >
                            {ORDER_ITEM_STATUS_MAP[normalizeOrderStatus(item.status)]?.label}
                          </Tag>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-semibold text-slate-900">
                          {formatCurrency(
                            (item.price || item.dish_id?.dish_price || 0) * item.quantity
                          )}
                        </p>
                      </div>
                    </div>
              ...
              <div className="rounded-2xl bg-orange-50 p-6 border border-orange-200 flex justify-between items-center shadow-sm">
                ...
                <span className="text-2xl font-black text-orange-600">
                  {formatCurrency(servedGrandTotal)}
                </span>
              </div>
            </>
          )}
        ...
        {flattenedItems.length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 border-t border-slate-200 bg-white/95 backdrop-blur-xl p-4 shadow-lg z-40">
            <div className="mx-auto max-w-6xl flex gap-3">
              <button
                onClick={() => navigate(`/table-order/${tableId}/menu`)}
                className="flex-1 h-12 rounded-lg border border-slate-300 bg-white font-semibold text-slate-700 hover:bg-slate-50 transition-all"
              >
                Gọi thêm món
              </button>
              <button
                ...
                disabled={!canPay}
                title={!canPay ? 'Không thể thanh toán: còn món đang xử lý' : ''}
                className={`flex-1 h-12 rounded-lg ${canPay ? 'bg-linear-to-r from-orange-500 to-orange-600 text-white font-bold hover:shadow-lg' : 'bg-slate-100 text-slate-400 cursor-not-allowed'} transition-all`}
              >
                Thanh toán
              </button>
            </div>
        ...
        <Modal
          title={
            showSepayQR ? (
              <div className="flex items-center gap-2">
                <QrcodeOutlined className="text-xl text-blue-500" />
                <span className="font-bold">Quét mã để thanh toán</span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
          ...
          okButtonProps={{
            className: 'bg-orange-500 font-bold h-10',
            ...
            <div className="mt-6">
              ...
              >
                <Space direction="vertical" className="w-full" style={{ gap: '12px' }}>
                  <div
                    className={`rounded-xl border-2 p-4 cursor-pointer transition-all ${paymentMethod === 'sepay' ? 'border-orange-500 bg-orange-50' : 'border-slate-200 hover:border-slate-300'}`}
                  >
                    <Radio value="sepay" className="w-full">
                      <div className="flex items-center gap-4">
                        <QrcodeOutlined className="text-xl text-blue-600" />
                        <div>
                          <p className="font-bold text-slate-900">VietQR (Tự động)</p>
                          <p className="text-xs text-slate-500">Xác nhận thanh toán ngay lập tức</p>
                        </div>
                      </div>
                    </Radio>
                  </div>
                  <div
                    className={`rounded-xl border-2 p-4 cursor-pointer transition-all ${paymentMethod === 'cash' ? 'border-orange-500 bg-orange-50' : 'border-slate-200 hover:border-slate-300'}`}
                  >
              ...
              <div className="mt-6 rounded-xl bg-amber-50 border border-amber-200 p-4">
                <p className="text-sm font-semibold text-amber-900">
                  Tổng thanh toán:{' '}
                  <span className="text-orange-600">{formatCurrency(servedGrandTotal)}</span>
                </p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col items-center py-6 animate-fadeIn">
              ...
              <div className="mt-6 flex items-center gap-2 text-blue-600 animate-pulse">
                ...
                <span className="text-sm font-semibold italic">
                  Đang chờ hệ thống xác nhận tiền...
                </span>
              </div>
            </div>
          )}
